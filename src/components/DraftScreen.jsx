...
export const DraftScreen = ({ draftPool, buffLevel, activePlayer, onSelectBuff, theme }) => {
    return (
        <div className={`h-screen w-screen flex flex-col items-center justify-center relative overflow-hidden transition-colors duration-500
        ...
        `}>
            ...
            <div className="z-10 text-center mb-12 animate-in slide-in-from-top-10 duration-700">
                <div className="flex items-center justify-center gap-3 mb-2">
                    ...
                    <h1 className="text-4xl font-black uppercase tracking-wider">Roguelike Draft</h1>
                    <Sparkles className="text-amber-400" size={32} />
                </div>
                <p className={`text-lg font-medium opacity-60`}>
                    Level {buffLevel} Buff Selection
                </p>
            </div>

            {/* Active Player Indicator */}
            <div className={`z-10 mb-8 flex items-center gap-3 px-6 py-3 rounded-full border shadow-lg animate-pulse
                ${activePlayer === 'p1' 
                    ...
                    >
                        ...
                        {buff.effects.winCondition && (
                            <div className="mt-auto pt-4 border-t border-white/10 text-xs space-y-1 opacity-90">
                                <p className="font-bold uppercase opacity-60 mb-1">Win Condition Shift:</p>
                                {buff.effects.winCondition.points && (
                                    <div className="flex justify-between">
                                        <span>Points:</span>
                                        <span className="font-mono font-bold text-amber-300">{buff.effects.winCondition.points}</span>
                                    </div>
                                )}
                                {buff.effects.winCondition.crowns && (
                                    <div className="flex justify-between">
                                        <span>Crowns:</span>
                                        <span className="font-mono font-bold text-amber-300">{buff.effects.winCondition.crowns}</span>
                                    </div>
                                )}
                        ...
                        {/* Select Action */}
                        <div className={`absolute bottom-6 right-6 opacity-0 group-hover:opacity-100 transition-all transform translate-y-2 group-hover:translate-y-0 flex items-center gap-2 font-bold uppercase tracking-widest text-xs
