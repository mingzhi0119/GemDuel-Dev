...
| `window.electron.refreshRuntimeRelayProfile()` | `refresh-runtime-relay-profile` | Renderer `invoke` -> Main `handle` | Desktop shell                | None                 | Refreshes the active short-lived TURN lease in the main process without exposing relay secrets to renderer.   | `Completed` |
| `window.electron.revokeRuntimeRelayProfile()`  | `revoke-runtime-relay-profile`  | Renderer `invoke` -> Main `handle` | Desktop shell                | None                 | Revokes the active short-lived TURN lease during renderer cleanup and returns to a fail-closed relay state.   | `Completed` |
...
- Production: `file://...`
- Payload policy:
- Zero-argument channels: `get-app-version`, `get-runtime-ice-servers`, `get-runtime-relay-profile`, `refresh-runtime-relay-profile`, `revoke-runtime-relay-profile`, `get-release-health-snapshot`, and `restart_app`.
