...
const readJson = (relativePath: string) => JSON.parse(readFile(relativePath));
type BuildReleaseHealthReportInput = Parameters<typeof buildReleaseHealthReport>[0];
...
describe('release-health operations governance', () => {
    ...
    it('reports drift when the operational snapshot or drill docs lose required entries', () => {
        ...
        const errors = collectReleaseHealthOperationsErrors({
            sloText: sloText.replace(/ipcRejected/g, 'ipcRejectedBroken'),
            drillText: drillText.replace(/`network-recovery`/g, 'network recovery'),
    ...
    it('reports granular schema, threshold, drill, artifact, and bundle budget drift', () => {
        const brokenSnapshot = {
            ...
            drills: operationsSnapshot.drills.map(
                (drill: (typeof operationsSnapshot.drills)[number]) => {
            ...
            artifactReports: [
                {
                    id: 'healthy-baseline',
                    sourcePath: 'tools/governance/release-health-fixtures/missing.jsonl',
                    expectedStatus: '',
                },
            ],
            bundleBudgets: {
        ...
        expect(errors).toEqual(
            expect.arrayContaining([
                ...
                'Artifact report healthy-baseline references missing source tools/governance/release-health-fixtures/missing.jsonl.',
...
describe('release-health report exporter', () => {
    it('derives a machine-readable report from raw release-health JSONL logs', () => {
        ...
        } as BuildReleaseHealthReportInput);
    ...
    it('exposes the CLI exporter as a runnable machine-readable tool', () => {
        ...
        try {
            ...
            const stdout = execFileSync(
                'node',
                ['tools/scripts/export-release-health-report.mjs', inputPath],
                {
                    cwd: repoRoot,
                    encoding: 'utf8',
                }
            ...
            expect(parsed.provenance.generatedBy).toBe(
    ...
    it('supports stdin, --out, and compact serialization through the CLI exporter', () => {
        ...
        try {
            const result = spawnSync(
                'node',
                ['tools/scripts/export-release-health-report.mjs', '--out', outPath, '--compact'],
                {
                    cwd: repoRoot,
                    encoding: 'utf8',
                    input: [
    ...
    it('accepts --pretty explicitly and exits non-zero when the CLI exporter receives invalid input', () => {
        ...
        try {
            ...
            const prettyResult = spawnSync(
                'node',
                ['tools/scripts/export-release-health-report.mjs', inputPath, '--pretty'],
                {
                    cwd: repoRoot,
                    encoding: 'utf8',
                }
