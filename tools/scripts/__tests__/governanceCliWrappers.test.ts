...
import { SEAL_COVERAGE_EXCLUSION_GOVERNANCE_POLICY } from '@gemduel/config-vitest/seal-exclusions';
...
const defaultDistDir = path.join(repoRoot, 'apps', 'desktop', 'dist');
...
describe('governance CLI wrappers', () => {
    ...
    it('uses the default governance artifact directory when the retained evidence is healthy', () => {
        withReplacedRepoPath(
            ...
            () => {
                const result = runNode(['tools/scripts/check-governance-evidence-health.mjs']);
    ...
    it('passes the seal exclusion wrapper with the current reviewed exclusion set', () => {
        const result = runNode(['tools/scripts/check-seal-exclusion-governance.mjs']);
    ...
    it('fails the seal exclusion wrapper when review timestamps have expired', () => {
        const result = runNode([
            'tools/scripts/check-seal-exclusion-governance.mjs',
            '--today',
            '2026-05-22',
        ]);
