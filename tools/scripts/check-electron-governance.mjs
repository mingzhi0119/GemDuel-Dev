...
} from '../../apps/desktop/electron/desktopGovernance.js';
import { GOVERNANCE_DOC_PATHS } from './governanceDocPaths.js';

const require = createRequire(import.meta.url);
const { createElectronBridge } = require('../../apps/desktop/electron/preloadContract.cjs');
...
const expectedSnapshot = JSON.parse(
    fs.readFileSync(
        path.join(repoRoot, 'tools', 'governance', 'desktop-policy.snapshot.json'),
        'utf8'
    )
);
const errors = collectDesktopGovernanceErrors({
    windowOptions: createMainWindowOptions({
        preloadPath: path.join(repoRoot, 'apps', 'desktop', 'electron', 'preload.js'),
