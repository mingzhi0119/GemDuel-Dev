...
export const useElectronUnityParityHarness = (params: UseElectronUnityParityHarnessParams) => {
    ...
    useEffect(() => {
        ...
        const advanceLoadedReplayEvent = async (
            ...
            currentParams().game.handlers.importHistory(
                loadedReplayHistory.slice(0, nextRevision + 1)
            );
        ...
        const clickPlayerReserved = async (
            action: ParityAction,
            payload: Record<string, unknown>
        ) => {
