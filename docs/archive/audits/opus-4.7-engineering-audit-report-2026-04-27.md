...
**审计方式:** 参照 `docs/archive/audits/engineering-audit-report-2026-04-27.md`、`docs/archive/audits/engineering-audit-report-2026-04-25.md`、`docs/archive/audits/opus-4.7-independent-audit-report-2026-04-21.md` 三份归档报告的模板与方法，执行仓库内完整工程门禁与治理脚本，按十个工程维度评分并输出 P0-P3 阻塞分级。
**审计边界:**
...
- **建议:** 当 `lifecycle-certification.report.json` 状态由 passed 转 failed 时，自动写一个 `docs/archive/audits/engineering-audit-draft-<date>.md` 草稿（包含 dashboard 失败指标、维度分数、建议动作模板），让"审计报告"成为"治理失败时的默认产物"，缩短人工整理路径。
- **预期收益:** 降低审计报告滚动迭代成本。
- **owner:** Release Engineering
