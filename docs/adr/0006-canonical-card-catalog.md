...
`packages/shared/src/data/realCards.ts` is large because it is the canonical in-repo card catalog, not because it mixes unrelated runtime behavior.
...
- The architecture budget gate permits `packages/shared/src/data/realCards.ts` up to `900` lines while the catalog remains authoritative in-repo data.
