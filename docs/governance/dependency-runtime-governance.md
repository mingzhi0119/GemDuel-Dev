...
| ------------------------------------- | ------------------- | ----------------------- | --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------- |
| `AGENT_BROWSER_BIN`                   | Client Parity       | `unset`                 | Optional absolute path or executable name for agent-browser migration automation  | Local tooling path only                                                        | Falls back to resolving `agent-browser` from `PATH` or the package shim                                    |
...
| `GEMDUEL_PARITY_BROWSER_FINAL_EXTRA`  | Client Parity       | `0`                     | Non-negative integer grace count for extra final agent-browser processes          | Operational parity-runner knob only                                            | Falls back to strict final process-count checks                                                            |
| `GEMDUEL_PARITY_BROWSER_PROCESS_MAX`  | Client Parity       | `4`                     | Non-negative integer maximum tolerated agent-browser processes during parity runs | Operational parity-runner knob only                                            | Falls back to the governed parity-runner process ceiling                                                   |
| `GEMDUEL_UNITY_PROJECT_PATH`          | Client Parity       | `clients/unity`         | Absolute or repository-relative Unity project path used by parity automation      | Local tooling path only                                                        | Falls back to the governed `clients/unity` path and records a Unity blocker if unavailable                 |
| `GEMDUEL_PNPM_PATH`                   | Client Parity       | `unset`                 | Absolute filesystem path to local `pnpm` for Unity LocalDev built-player smoke    | Local tooling path only                                                        | Falls back to `PATH` resolution; reports a structured bridge blocker if `pnpm` cannot launch               |
| `GEMDUEL_RULES_BRIDGE_MAILBOX_DIR`    | Client Parity       | `unset`                 | Ignored LocalDev request/response mailbox for Unity built-player smoke automation | Local tooling path only                                                        | Falls back to direct process launch; reports a structured bridge blocker if no bridge transport can run    |
...
| `UNITY_EXE`                           | Client Parity       | `unset`                 | Absolute filesystem path to the Unity Editor executable used by parity automation | Local tooling path only                                                        | Parity runner falls back to the governed default Unity Hub path and records a Unity blocker if unavailable |
| `PATH`                                | Client Parity       | inherited               | Operating-system executable search path used by local automation to discover pnpm | Local tooling path metadata only                                               | Built-player smoke omits `GEMDUEL_PNPM_PATH` and reports a structured pnpm availability blocker            |
