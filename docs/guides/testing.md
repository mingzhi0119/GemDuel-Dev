...
- When a gameplay term is normalized, add or update lexicon catalog tests, UI interaction tests, and a regression check that blocks legacy phrases from returning.
- Replay outputs written to `Replay/` are local artifacts for audit work and should remain untracked by git.
