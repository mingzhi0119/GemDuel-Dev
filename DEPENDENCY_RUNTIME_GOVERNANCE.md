...
| `GEMDUEL_TURN_SERVICE_URL`            | `Completed` | Networking          | `unset`             | Must be an absolute `http:` or `https:` URL for the governed TURN credential service.                                                                                                                     | Endpoint metadata only. Keep the bearer token separate and never emit the resolved URL in release-health payloads.                                              | Disables online TURN fetch and falls back to governed runtime relay sources if missing or invalid.                        |
| `GEMDUEL_TURN_SERVICE_TOKEN`          | `Completed` | Networking          | `unset`             | Opaque bearer token required before the desktop runtime may issue, refresh, or revoke short-lived TURN credentials.                                                                                       | Treat as a secret. It must not be committed, logged, or exposed to renderer code.                                                                               | Disables online TURN fetch and falls back to governed runtime relay sources unless fallback deny is enabled.              |
| `GEMDUEL_TURN_SERVICE_FALLBACK_MODE`  | `Completed` | Networking          | `allow-runtime-ice` | Must be one of `allow-runtime-ice` or `deny-runtime-ice`.                                                                                                                                                 | Operational policy only. Never store relay credentials here.                                                                                                    | Falls back to `allow-runtime-ice` if the configured value is malformed.                                                   |
...
- Status: `In Progress`
- Current state: the repo now includes an authenticated short-lived TURN credential service contract plus desktop-side online fetch, refresh, and revoke lifecycle. The long-lived runtime bundle remains a governed fallback path while deployments migrate.
- Required end state:
...
- `src/app/runtime/useRuntimeAppConfig.ts` now records whether the renderer loaded a preferred relay profile, refreshed it near expiry, or fell back to the legacy ICE bridge.
